...
AGENT_ID=$(printf '%s' "$resp_agent" | python3 - <<'PY'
import json,sys
obj=json.load(sys.stdin)
print(obj["agentId"])
PY
)
AGENT_SECRET=$(printf '%s' "$resp_agent" | python3 - <<'PY'
import json,sys
obj=json.load(sys.stdin)
print(obj["agentSecret"])
PY
)

echo "Creating primary tribe (${TRIBE_NAME})"
resp_tribe=$(json_post "/api/tribes/create" "{\"name\":\"${TRIBE_NAME}\",\"leaderAgentId\":\"${AGENT_ID}\"}")
TRIBE_ID=$(printf '%s' "$resp_tribe" | python3 - <<'PY'
import json,sys
obj=json.load(sys.stdin)
print(obj["tribeId"])
PY
...
OPP_AGENT_ID=$(printf '%s' "$resp_opp_agent" | python3 - <<'PY'
import json,sys
obj=json.load(sys.stdin)
print(obj["agentId"])
PY
...
OPP_TRIBE_ID=$(printf '%s' "$resp_opp_tribe" | python3 - <<'PY'
import json,sys
obj=json.load(sys.stdin)
print(obj["tribeId"])
PY
...
JOUST_ID=$(printf '%s' "$resp_joust" | python3 - <<'PY'
import json,sys
obj=json.load(sys.stdin)
print(obj["joustId"])
PY
